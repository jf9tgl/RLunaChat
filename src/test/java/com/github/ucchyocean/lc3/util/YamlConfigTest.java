...
public class YamlConfigTest extends TestCase {
    ...
    public void testYamlConfig() {
        ...
        assertEquals("test", config.getString("aaa.bbb"));
        assertEquals("testdesu", config.getString("bbb.aiueo.aaa"));
        assertEquals(1, config.getInt("aaa.int"));
        assertEquals(2, config.getStringList("bbb.list").size());
    }
}
